...
这里维护一个自己走过的路径数组，如果之前有走过，就回溯回之前状态，然后往其他方向走
...
public:
    bool hasPath(char* array, int rows, int cols, char* str)
    {
        ...
        vector<vector<char>> matrix = buildMatrix(array);
        string s=str;
        for (int i = 0; i < rows; i++)
            for (int j = 0; j < cols; j++)
                if (backtracking(matrix, s, marked, 0, i, j))
                    return true;
...
private:
    vector<vector<char>> buildMatrix(char* array) {
    ...
    bool backtracking(vector<vector<char>> matrix, string str, vector<vector<bool>> marked, int pathLen, int r, int c) {
        if (pathLen == str.size()) return true;
